...
inline AggregatedTopologyState AggregateState(const FairMQTopologyState& topologyState)
{
    ...
    if (std::all_of(topologyState.cbegin(), topologyState.cend(), [&](FairMQTopologyState::value_type i) {
...
class BasicTopology : public AsioBase<Executor, Allocator>
{
  public:
    ...
    {
        fDDSCustomCmd.subscribe([&](const std::string& msg, const std::string& /* condition */, DDSChannel::Id senderId) {
            cc::Cmds inCmds;
            inCmds.Deserialize(msg);
            // OLOG(ESeverity::debug) << "Received " << inCmds.Size() << " command(s) with total size of " << msg.length() << " bytes: ";
        ...
        if (cmd.GetResult() == cc::Result::Ok) {
            ...
            try {
                ...
                if (!task.subscribed_to_state_changes) {
                    ...
                    OLOG(ESeverity::warning) << "Task '" << task.taskId << "' sent subscription confirmation more than once";
                }
                lk.unlock();
                fStateChangeSubscriptionsCV->notify_one();
            } catch (const std::exception& e) {
                OLOG(ESeverity::error) << "Exception in HandleCmd(cc::StateChangeSubscription const&): " << e.what();
                OLOG(ESeverity::error) << "Possibly no task with id '" << taskId << "'?";
            }
        } else {
            OLOG(ESeverity::error) << "State change subscription failed for device: " << cmd.GetDeviceId() << ", task id: " << cmd.GetTaskId();
        }
    }
        ...
        if (cmd.GetResult() == cc::Result::Ok) {
            ...
            try {
                ...
                if (task.subscribed_to_state_changes) {
                    ...
                    OLOG(ESeverity::warning) << "Task '" << task.taskId << "' sent unsubscription confirmation more than once";
                }
                lk.unlock();
                fStateChangeSubscriptionsCV->notify_one();
            } catch (const std::exception& e) {
                OLOG(ESeverity::error) << "Exception in HandleCmd(cc::StateChangeUnsubscription const&): " << e.what();
            }
        } else {
            OLOG(ESeverity::error) << "State change unsubscription failed for device: " << cmd.GetDeviceId() << ", task id: " << cmd.GetTaskId();
        }
    }
    ...
    auto HandleCmd(cc::StateChange const& cmd, DDSChannel::Id const& senderId) -> void
    {
        if (cmd.GetCurrentState() == DeviceState::Exiting) {
            fDDSCustomCmd.send(cc::Cmds(cc::make<cc::StateChangeExitingReceived>()).Serialize(), std::to_string(senderId));
        }
            ...
            for (auto& op : fChangeStateOps) {
                if (!op.second.IsCompleted() && op.second.ContainsTask(taskId)) {
                    if (fStateData.at(fStateIndex.at(taskId)).state != op.second.GetTargetState()) {
                        OLOG(ESeverity::error) << cmd.GetTransition() << " transition failed for " << cmd.GetDeviceId() << ", device is in " << cmd.GetCurrentState() << " state.";
                        op.second.Complete(MakeErrorCode(ErrorCode::DeviceChangeStateFailed));
                    } else {
                        OLOG(ESeverity::debug) << cmd.GetTransition() << " transition failed for " << cmd.GetDeviceId() << ", device is already in " << cmd.GetCurrentState() << " state.";
                    }
                }
            }
        }
  ...
  private:
    struct ChangeStateOp
    {
            ...
            , fMtx(mutex)
        {
            ...
            if (fTasks.empty()) {
                OLOG(ESeverity::warning) << "ChangeState initiated on an empty set of tasks, check the path argument.";
            }
        }
  ...
  public:
    ...
    {
        return boost::asio::async_initiate<CompletionToken, ChangeStateCompletionSignature>([&](auto handler) {
        ...
        -> std::pair<std::error_code, FairMQTopologyState>
    {
        ...
        AsyncChangeState(transition, path, timeout, [&, blocker](std::error_code _ec, FairMQTopologyState _state) mutable {
            ec = _ec;
            state = _state;
            blocker.Signal();
        });
  ...
  private:
    struct WaitForStateOp
    {
            ...
            , fMtx(mutex)
        {
            ...
            if (fTasks.empty()) {
                OLOG(ESeverity::warning) << "WaitForState initiated on an empty set of tasks, check the path argument.";
            }
        }
  ...
  public:
    ...
    auto AsyncWaitForState(const DeviceState targetLastState,
                           ...
                           CompletionToken&& token)
    {
        return boost::asio::async_initiate<CompletionToken, WaitForStateCompletionSignature>([&](auto handler) {
  ...
  private:
    struct GetPropertiesOp
    {
            ...
            , fMtx(mutex)
        {
            ...
            if (expectedCount == 0) {
                OLOG(ESeverity::warning) << "GetProperties initiated on an empty set of tasks, check the path argument.";
            }
            // OLOG(ESeverity::debug) << "GetProperties " << fId << " with expected count of " << fExpectedCount << " started.";
        }
  ...
  private:
    struct SetPropertiesOp
    {
            ...
            , fMtx(mutex)
        {
            ...
            if (expectedCount == 0) {
                OLOG(ESeverity::warning) << "SetProperties initiated on an empty set of tasks, check the path argument.";
            }
            // OLOG(ESeverity::debug) << "SetProperties " << fId << " with expected count of " << fExpectedCount << " started.";
        }
