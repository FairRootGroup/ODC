...
int main(int argc, char** argv)
{
    try
    {
        ...
        options.add_options()("res",
                              bpo::value<string>(&res)->default_value("{\"zone\":\"online\",\"n\":1}"),
        ...
        options.add_options()("bash",
                              bpo::value<string>(&bash)->default_value(""),
                              "Optional bash commands to be prepended to SSH config file");
        options.add_options()(
        ...
        if (!release)
        {
            ...
            for (const auto& v : r.m_resources)
            {
                ...
                OLOG(ESeverity::info) << "epnc: nodes requested (" << v.m_n << ") allocated (" << nodes.size() << ")"
                                      << endl;
